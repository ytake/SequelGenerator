...
namespace Ytake\SequelGenerator\Writer\Database\Mysql;

use Ytake\SequelGenerator\Writer\Database\SchemeInterface;
use Ytake\SequelGenerator\Writer\Database\Mysql\Templates\Database;

/**
 * Class Scheme
 *
 * @package Ytake\SequelGenerator\Writer\Database\Mysql
 */
            ...
            foreach ($indexes as $row) {
                if ($row['index_name'] != '') {
                    if ($row['index_type'] == "FULLTEXT INDEX" && $engine == "MyISAM") {
                        $index[] = "{$row['index_type']} {$row['index_name']} (" . implode(',',
                                $row['filed_names']) . ")";
                    } else {
                        $index[] = "{$row['index_type']} {$row['index_name']} (" . implode(',',
                                $row['filed_names']) . ")";
